...
%stimulations for serotonin for figure S1M-S1P
...
FigPath = fullfile('');
%Set Files 
NT = string({'Figure_S1M_to_P_emostroop_rodent_NE.mat'});
...
Null_Predictions{3,1}(:,2) = Pred_3_z{nulls_3(2),1}(:,1);
Null_Predictions{3,1}(:,3) = Pred_3_z{nulls_3(3),1}(:,1);
...
Null_Predictions_SE{3,1}(:,2) = Pred_3_z{nulls_3(2),1}(:,2);
Null_Predictions_SE{3,1}(:,3) = Pred_3_z{nulls_3(3),1}(:,2);
...
Null_Predictions_NE{3,1}(:,2) = Pred_3_z{nulls_3(2),1}(:,4);
Null_Predictions_NE{3,1}(:,3) = Pred_3_z{nulls_3(3),1}(:,4);
...
fig = figure
    ...
    plot((1:length(Across_Null_Final_z_NE))/fs,Across_Null_Final_z_NE,'LineWidth',1,'Color',"#8A2BE2"); 

    shadedErrorBar((1:length(Across_Stim_Final_z_NE))/fs,Across_Stim_Final_z_NE,Animals_Stim_SEM_z_NE,'lineProps','c');
    shadedErrorBar((1:length(Across_Null_Final_z_NE))/fs,Across_Null_Final_z_NE,Animal_Null_SEM_z_NE,'lineProps',{'-','markerfacecolor',	"#8A2BE2",'color',	"#8A2BE2"} );
...
xlim([0 22])
  ...
  saveas(gcf,fullfile(FigPath,'Figure_S1M'),'jpeg');
...
saveas(gcf,fullfile(FigPath,'Figure_S1N'),'jpeg');
...
Graph_AUC_svn(1,1) = mean(AUC_NE(:,1),'omitnan');
Graph_AUC_svn(1,2) = mean(AUC_NE_null(:,1),'omitnan');


Graph_AUC_SEM_svn(1,1) = (std(AUC_NE(:,1),1,'omitnan'))/(sqrt(length(find(~isnan(AUC_NE(:,1))))));
Graph_AUC_SEM_svn(1,2) = (std(AUC_NE_null(:,1),1,'omitnan'))/(sqrt(length(find(~isnan(AUC_NE_null(:,1))))));
...
NE_num = size(AUC_NE,1);
NE_num_null = size(AUC_NE_null,1);

x2 = ones(1,NE_num);
x3 = ones(1,NE_num_null)*2;
figure
hold on
...
scatter(x2,AUC_NE,markersize, 'MarkerEdgeColor','k','MarkerFaceColor',[0.5000    0.5000    1.0000]);
scatter(x3,AUC_NE_null,markersize,'MarkerEdgeColor','k','MarkerFaceColor',[0.5000    0.5000    1.0000]);



b.CData(1,:) = [0 1 1];	%%cyan	
b.CData(2,:) = [138, 43, 226] / 255; %%null color
    ...
    ylabel('NE AUC [z]','Color','k');
    ax = gca;
    ax.FontSize = 25;
    axis square 
   set(gca, 'Units', 'inches','Position', [2, 2, 1.2, 1.2], 'color', 'none', 'FontName', 'Arial', 'FontSize' , 8 ); 
    
   saveas(gcf,fullfile(FigPath,'Figure_S1O'),'jpeg');
...
b.CData(3,:) = [0 1 1];
    ...
    saveas(gcf,fullfile(FigPath,'Figure_S1P'),'jpeg');
