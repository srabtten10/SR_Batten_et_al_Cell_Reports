...
%Run linear mixed-effects analysis (Model 9) and plots Figure S8A through S8D
...
for cc = 1:size(PickSub,2)
    if NT_Final(PickSub(cc)).Region == 'LVIM'
        Hemisphere{cc,1} = repelem(0,176)'; 
    else 
        Hemisphere{cc,1} = repelem(1,176)';  
    end
end

for cc = 1:size(PickSub,2)
    Hemisphere_vert{cc,1} = vertcat(Hemisphere{cc,1});
end

Model_Hemisphere = vertcat(Hemisphere_vert{:});
...
for cc = 1:size(pos_Peak_Change,1)
    ...
    Peak_Subject{cc,gg} = vertcat(pos_Peak_Change{cc,gg},neg_Peak_Change{cc,gg},neut_Peak_Change{cc,gg});
    end
end
...
datanames   = {'Subject','RxTime','Hemisphere',...
               'Peak','Valence_Num','Arousal_Num'};
for cc = 1:size(Model_Peak_Change,2)    
    Model_Table = table(categorical(Model_Subject),Model_Rx_Time,categorical(Model_Hemisphere),Model_Peak_Change(:,cc),...
...
%With Random Effects
formulaz = 'Peak ~ 1 + (Valence_Num + Arousal_Num) * Hemisphere * RxTime + (1 + (Valence_Num + Arousal_Num) * Hemisphere * RxTime|Subject)';
lme_Relative_AUC = fitglme(Model_Table,formulaz,'distribution','normal','Link','identity','FitMethod','Laplace');
%Without Random Effects
formulaz = 'Peak ~ 1 + (Valence_Num + Arousal_Num) * Hemisphere * RxTime + (1|Subject)';
...
Relative_Model_Names = cell({'Peak ~ 1 + (Valence_Num + Arousal_Num) * Hemisphere * RxTime + (1 + (Valence_Num + Arousal_Num)* RxTime * Hemisphere|Subject)'});
                         
Relative_NoRand_Model_Names = cell({'Peak ~ 1 + (Valence_Num + Arousal_Num) * Hemisphere * RxTime + (1|Subject)'});
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Pos(cc,1) = mean(Peak_Subject{cc,1}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,2) = mean(Peak_Subject{cc,2}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,3) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    
    
    Neut(cc,1) = mean(Peak_Subject{cc,1}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Neut(cc,2) = mean(Peak_Subject{cc,2}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Neut(cc,3) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
end


%DA Mean
mean_DA(2,1) = mean(Pos(:,1),'omitnan');
mean_DA(1,1) = mean(Neut(:,1),'omitnan');

%Group SEM:DA
SEM_DA(2,1) = std(Pos(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,1))))));
SEM_DA(1,1) = std(Neut(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neut(:,1))))));

%SE Mean
mean_SE(2,1) = mean(Pos(:,2),'omitnan');
mean_SE(1,1) = mean(Neut(:,2),'omitnan');

%Group SEM:SE
SEM_SE(2,1) = std(Pos(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,2))))));
SEM_SE(1,1) = std(Neut(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Neut(:,2))))));

%NE Mean
mean_NE(2,1) = mean(Pos(:,3),'omitnan');
mean_NE(1,1) = mean(Neut(:,3),'omitnan');

%Group SEM:NE
SEM_NE(2,1) = std(Pos(:,3),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,3))))));
SEM_NE(1,1) = std(Neut(:,3),1,'omitnan')/(sqrt(length(find(~isnan(Neut(:,3))))));


subplot(1,3,2)
hold on
errorbar(1:2,mean_NE, SEM_NE, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,mean_NE, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'c', 'MarkerSize', 30);
hold off

    xlim([0 3])
    ylim([-Yaxis  Yaxis]);
    xticks([1 2]);
    xticklabels({'Neutral' 'Arousal'});
    xtickangle(45);
    ylabel('Max Change [z]');
    yline(0,'--','LineWidth', 2)
    ax = gca;
...
saveas(gcf,fullfile(FigPath,'Figure_S8A'),'jpeg');
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Pos(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    
    Neg(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Neg(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Neg(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    
    Neut(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == 0 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Neut(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == 0 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    Neut(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == 0 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
end


%DA Mean
mean_DA(2,1) = mean(Pos(:,1),'omitnan');
mean_DA(1,1) = mean(Neg(:,1),'omitnan');

%Group SEM:DA
SEM_DA(2,1) = std(Pos(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,1))))));
SEM_DA(1,1) = std(Neg(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,1))))));


%SE Mean
mean_SE(2,1) = mean(Pos(:,2),'omitnan');
mean_SE(1,1) = mean(Neg(:,2),'omitnan');

%Group SEM:SE
SEM_SE(2,1) = std(Pos(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,2))))));
SEM_SE(1,1) = std(Neg(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,2))))));


%NE Mean
mean_NE(2,1) = mean(Pos(:,3),'omitnan');
mean_NE(1,1) = mean(Neg(:,3),'omitnan');

%Group SEM:NE
SEM_NE(2,1) = std(Pos(:,3),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,3))))));
SEM_NE(1,1) = std(Neg(:,3),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,3))))));


subplot(1,3,2)
hold on
errorbar(1:2,mean_DA, SEM_DA, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,mean_DA, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'k', 'MarkerSize', 30);

errorbar(1:2,mean_SE, SEM_SE, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,mean_SE, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'm', 'MarkerSize', 30);
hold off
...
saveas(gcf,fullfile(FigPath,'Figure_S8B'),'jpeg');
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Pos(cc,1) = mean(Peak_Subject{cc,1}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Pos(cc,2) = mean(Peak_Subject{cc,2}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Pos(cc,3) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    
    
    Neut(cc,1) = mean(Peak_Subject{cc,1}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neut(cc,2) = mean(Peak_Subject{cc,2}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neut(cc,3) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
end
...
saveas(gcf,fullfile(FigPath,'Figure_S8C'),'jpeg');
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Pos(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Pos(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Pos(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    
    Neg(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neg(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neg(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    
    Neut(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == 0 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neut(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == 0 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neut(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == 0 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
end
...
saveas(gcf,fullfile(FigPath,'Figure_S8D'),'jpeg');
