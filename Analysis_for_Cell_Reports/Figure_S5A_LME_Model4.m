...
%Run linear mixed-effects analysis (Model 4) and plots Figure S5A
...
%Get indexes 
for cc = 1:size(PickSub,2)
    for rr = 1:size(Rx_Times_vert,2)
    Aro_Rx(cc,rr) = mean(Rx_Times_vert{cc,rr}(salience_vert{cc,1} == 1,1),'omitnan');
    Neut_Rx(cc,rr) = mean(Rx_Times_vert{cc,rr}(salience_vert{cc,1} == -1,1),'omitnan');
    end
end

scatterX1 = repelem(2,length(Aro_Rx(:,1)));
scatterX2 = repelem(1,length(Neut_Rx(:,1)));


%DA Mean

mean_Rx(2,1) = mean(Aro_Rx(:,1),'omitnan');
mean_Rx(1,1) = mean(Neut_Rx(:,1),'omitnan');
%Group SEM:DA

SEM_Rx(2,1) = std(Aro_Rx(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Aro_Rx(:,1))))));
SEM_Rx(1,1) = std(Neut_Rx(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neut_Rx(:,1))))));


subplot(1,3,1)
b = bar(1:2,mean_Rx, 'FaceColor', 'flat');
hold on
er = errorbar(1:2,mean_Rx, SEM_Rx, 'LineStyle', 'none');
scatter(scatterX1,Aro_Rx(:,1),200,'MarkerEdgeColor', 'k','MarkerFaceColor',[0.7 0.7 0.7]);
scatter(scatterX2,Neut_Rx(:,1),200,'MarkerEdgeColor', 'k','MarkerFaceColor',[0.7 0.7 0.7]);
hold off
...
saveas(gcf,fullfile(FigPath,'Figure_S5A_ReactionTime_Arousal'),'jpeg');
