...
%Run linear mixed-effects analysis (Model 5) and plots Figure S7A
...
%% Make valence plots with DA, 5HT, NE; Figure 27A
...
%DA Mean
mean_DA(3,1) = mean(Pos(:,1),'omitnan');
mean_DA(1,1) = mean(Neg(:,1),'omitnan');
mean_DA(2,1) = mean(Neut(:,1),'omitnan');

%Group SEM:DA
SEM_DA(3,1) = std(Pos(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,1))))));
SEM_DA(1,1) = std(Neg(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,1))))));
SEM_DA(2,1) = std(Neut(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neut(:,1))))));

%SE Mean
mean_SE(3,1) = mean(Pos(:,2),'omitnan');
mean_SE(1,1) = mean(Neg(:,2),'omitnan');
mean_SE(2,1) = mean(Neut(:,2),'omitnan');
%Group SEM:SE
SEM_SE(3,1) = std(Pos(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,2))))));
SEM_SE(1,1) = std(Neg(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,2))))));
SEM_SE(2,1) = std(Neut(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Neut(:,2))))));

%NE Mean
mean_NE(3,1) = mean(Pos(:,3),'omitnan');
mean_NE(1,1) = mean(Neg(:,3),'omitnan');
mean_NE(2,1) = mean(Neut(:,3),'omitnan');
%Group SEM:NE
SEM_NE(3,1) = std(Pos(:,3),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,3))))));
SEM_NE(1,1) = std(Neg(:,3),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,3))))));
SEM_NE(2,1) = std(Neut(:,3),1,'omitnan')/(sqrt(length(find(~isnan(Neut(:,3))))));

subplot(1,5,1)

hold on
errorbar(1:3,mean_NE, SEM_NE, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:3,mean_NE, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'c', 'MarkerSize', 20);

errorbar(1:3,mean_DA, SEM_DA, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:3,mean_DA, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'k', 'MarkerSize', 20);

errorbar(1:3,mean_SE, SEM_SE, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:3,mean_SE, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'm', 'MarkerSize', 20);
hold off
...
saveas(gcf,fullfile(FigPath,'Figure_S7A_Blocked_ACC_DA_5HT_NE'),'jpeg');
