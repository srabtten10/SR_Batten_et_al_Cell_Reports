...
%Run linear mixed-effects analysis (Model 8) for dopamine and serotonin for
%positive valence (for plots Figure 3C and 3D)
...
for cc = 1:size(Rx_Time_Positive,1)
    Rx_Times_vert{cc,1} = vertcat(Rx_Time_Positive{cc,1},Rx_Time_Positive{cc,1});
end
...
for cc = 1:size(PickSub,2)
    valence_vert{cc,1} = vertcat(pos{cc,1},pos{cc,1});
end
...
for cc = 1:size(PickSub,2)
    accuracy_vert{cc,1} = vertcat(pos_acc{cc,1},pos_acc{cc,1});
end
...
for cc = 1:size(PickSub,2)
    DA{cc,1} = repelem(1,44)';
    SE{cc,1} = repelem(2,44)';
    %NE{cc,1} = repelem(3,44)';
end

for cc = 1:size(PickSub,2)
    NT_vert{cc,1} = vertcat(DA{cc,1},SE{cc,1});
end

Model_NT = vertcat(NT_vert{:});
...
for cc = 1:size(pos_Peak_Change,1)
    Peak_Subject{cc,1} = vertcat(pos_Peak_Change{cc,1},...
                                 pos_Peak_Change{cc,2});
end


Model_Peak_Change = vertcat(Peak_Subject{:,1});

%% Run LME for dopamine and serotonin to negative words 

datanames   = {'Subject','NT','RxTime'...
               'Peak'};
    
    Model_Table = table(categorical(Model_Subject),...
                        categorical(Model_NT),Model_Rx_Time,Model_Peak_Change,'VariableNames',datanames);


%With Random Effects
formulaz = 'Peak ~ 1 + NT * RxTime + (1 + NT * RxTime|Subject)';
lme_Relative_AUC = fitglme(Model_Table,formulaz,'distribution','normal','Link','identity','FitMethod','Laplace');
%Without Random Effects
formulaz = 'Peak ~ 1 + NT * RxTime + (1|Subject)';
...
Relative_Model_Names = cell({'Peak ~ 1 + NT * RxTime + (1 + NT * RxTime|Subject)'});
                         
Relative_NoRand_Model_Names = cell({'Peak ~ 1 + NT * RxTime + (1|Subject)'});
...
for cc = 1:size(Final_Peak_NoRand_Models,1)
    Model_Outputs{cc,1} = anova(Final_Peak_NoRand_Models{cc,2});
end

for cc = 1:size(Model_Outputs,1)
    for rr = 1:size(Model_Outputs{cc,1},1)
        for gg = 1:size(Model_Outputs{cc,1},2)
            Model_Outputs_Refined{cc,1}{rr,gg} = Model_Outputs{cc,1}{rr,gg};
        end
    end
end

for cc = 1:size(Model_Outputs_Refined,1)
    Model_Outputs_Table{cc,1} = cell2table(Model_Outputs_Refined{cc,1});
end
