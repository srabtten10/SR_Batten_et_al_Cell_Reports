...
%Run linear mixed-effects analysis (Model 9) and plots Figure S5B
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Neut(cc,1) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neut(cc,2) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    
    
    Aro(cc,1) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Aro(cc,2) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    
end


%DA Mean
mean_L(2,1) = mean(Neut(:,1),'omitnan');
mean_L(1,1) = mean(Aro(:,1),'omitnan');

%Group SEM:DA
SEM_L(2,1) = std(Neut(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,1))))));
SEM_L(1,1) = std(Aro(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,1))))));

%SE Mean
mean_R(2,1) = mean(Neut(:,2),'omitnan');
mean_R(1,1) = mean(Aro(:,2),'omitnan');

%Group SEM:SE
SEM_R(2,1) = std(Neut(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,2))))));
SEM_R(1,1) = std(Aro(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,2))))));

subplot(1,3,1)
hold on
errorbar(1:2,mean_L, SEM_L, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,mean_L, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'b', 'MarkerSize', 30);

errorbar(1:2,mean_R, SEM_R, 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,mean_R, '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', '#FFEF00', 'MarkerSize', 30);
hold off
...
saveas(gcf,fullfile(FigPath,'Figure_S5B_DA_ACC'),'jpeg');