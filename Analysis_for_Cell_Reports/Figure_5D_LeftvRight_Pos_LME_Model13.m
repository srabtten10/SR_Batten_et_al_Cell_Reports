...
%Run linear mixed-effects analysis (Model 13) for plots Figure 5D to look
%at left v right hemisphere in positive valence
...
for cc = 1:size(Rx_Time_Positive,1)
    Rx_Times_vert{cc,1} = vertcat(Rx_Time_Positive{cc,1});
end
...
for cc = 1:size(PickSub,2)
    Model_Subject = repelem(PickSub,44)'; %Put in LME format
end
...
for cc = 1:size(PickSub,2)
    valence_vert{cc,1} = vertcat(pos{cc,1});
end
...
for cc = 1:size(PickSub,2)
    salience_vert{cc,1} = vertcat(sal_1{cc,1},sal_2{cc,1});
end
...
for cc = 1:size(PickSub,2)
    accuracy_vert{cc,1} = vertcat(pos_acc{cc,1});
end
...
for cc = 1:size(PickSub,2)
    if NT_Final(PickSub(cc)).Region == 'LACC'
        Hemisphere{cc,1} = repelem(0,44)'; 
    else 
        Hemisphere{cc,1} = repelem(1,44)';  
    end
end
...
for cc = 1:size(pos_Peak_Change,1)
    ...
    Peak_Subject{cc,gg} = vertcat(pos_Peak_Change{cc,gg});
    end
end
...
%With Random Effects
formulaz = 'Peak ~ 1 + Hemisphere * RxTime + (1 + Hemisphere * RxTime|Subject)';
lme_Relative_AUC = fitglme(Model_Table,formulaz,'distribution','normal','Link','identity','FitMethod','Laplace');
%Without Random Effects
formulaz = 'Peak ~ 1 + Hemisphere * RxTime + (1|Subject)';
...
Relative_Model_Names = cell({'Peak ~ 1 + Hemisphere * RxTime + (1 + Valence * RxTime|Subject)'});
                         
Relative_NoRand_Model_Names = cell({'Peak ~ 1 + Hemisphere * RxTime + (1|Subject)'});
