...
%Run linear mixed-effects analysis (Model 9) and plots Figure S6A and S6B
...
for cc = 1:size(PickSub,2)
    if NT_Final(PickSub(cc)).Region == 'LACC'
...
for cc = 1:size(Rx_Times_vert,1)
    Qantiles{cc,1} = quantile(Rx_Times_vert{cc,1},2);
end

%Get indexes 
for cc = 1:size(PickSub,2)
    for rr = 1:size(Peak_Subject,2)
    Pos_Fast(cc,rr) = mean(Peak_Subject{cc,rr}(Rx_Times_vert{cc,1} < Qantiles{cc,1}(1,1) & valence_vert{cc,1} == 1,1),'omitnan');
    Pos_Slow(cc,rr) = mean(Peak_Subject{cc,rr}(Rx_Times_vert{cc,1} > Qantiles{cc,1}(1,2) & valence_vert{cc,1} == 1,1),'omitnan');

    Neut_Fast(cc,rr) = mean(Peak_Subject{cc,rr}(Rx_Times_vert{cc,1} < Qantiles{cc,1}(1,1) & valence_vert{cc,1} == 0,1),'omitnan');
    Neut_Slow(cc,rr) = mean(Peak_Subject{cc,rr}(Rx_Times_vert{cc,1} > Qantiles{cc,1}(1,2) & valence_vert{cc,1} == 0,1),'omitnan');
    
    Neg_Fast(cc,rr) = mean(Peak_Subject{cc,rr}(Rx_Times_vert{cc,1} < Qantiles{cc,1}(1,1) & valence_vert{cc,1} == -1,1),'omitnan');
    Neg_Slow(cc,rr) = mean(Peak_Subject{cc,rr}(Rx_Times_vert{cc,1} > Qantiles{cc,1}(1,2) & valence_vert{cc,1} == -1,1),'omitnan');
    end
end

for cc = 1:size(Pos_Slow,2)

%DA Mean
Pos(1,cc) = mean(Pos_Fast(:,cc),'omitnan');
Pos(2,cc) = mean(Pos_Slow(:,cc),'omitnan');

%Group SEM:DA
SEM_Pos(1,cc) = std(Pos_Fast(:,cc),1,'omitnan')/(sqrt(length(find(~isnan(Pos_Slow(:,cc))))));
SEM_Pos(2,cc) = std(Pos_Slow(:,cc),1,'omitnan')/(sqrt(length(find(~isnan(Pos_Fast(:,cc))))));

%SE Mean
Neg(1,cc) = mean(Neg_Fast(:,cc),'omitnan');
Neg(2,cc) = mean(Neg_Slow(:,cc),'omitnan');

%Group SEM:SE
SEM_Neg(1,cc) = std(Neg_Fast(:,cc),1,'omitnan')/(sqrt(length(find(~isnan(Neg_Slow(:,cc))))));
SEM_Neg(2,cc) = std(Neg_Slow(:,cc),1,'omitnan')/(sqrt(length(find(~isnan(Neg_Fast(:,cc))))));

%NE Mean
Neut(1,cc) = mean(Neut_Fast(:,cc),'omitnan');
Neut(2,cc) = mean(Neut_Slow(:,cc),'omitnan');

%Group SEM:NE
SEM_Neut(1,cc) = std(Neut_Fast(:,cc),1,'omitnan')/(sqrt(length(find(~isnan(Neut_Slow(:,cc))))));
SEM_Neut(2,cc) = std(Neut_Slow(:,cc),1,'omitnan')/(sqrt(length(find(~isnan(Neut_Fast(:,cc))))));

end

subplot(1,3,1)
hold on
errorbar(1:2,Neg(:,2), SEM_Neg(:,2), 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,Neg(:,2), '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'm', 'MarkerSize', 30);
hold off
...
subplot(1,3,2)
hold on
errorbar(1:2,Neut(:,2), SEM_Neut(:,2), 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,Neut(:,2), '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'm', 'MarkerSize', 30);
hold off
...
subplot(1,3,3)
hold on
errorbar(1:2,Pos(:,2), SEM_Pos(:,2), 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,Pos(:,2), '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'm', 'MarkerSize', 30);
hold off
...
saveas(gcf,fullfile(FigPath,'Figure_S6A_5HT_RxTime'),'jpeg');
...
hold on
errorbar(1:2,Neg(:,2), SEM_Neg(:,2), 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,Neg(:,2), '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'r', 'MarkerSize', 30);

errorbar(1:2,Pos(:,2), SEM_Pos(:,2), 'LineStyle', 'none', 'Color', 'k', 'LineWidth', 1,'CapSize',0);
plot(1:2,Pos(:,2), '-o', 'LineWidth', 2, 'Color', 'k', 'MarkerFaceColor', 'g', 'MarkerSize', 30);
hold off
...
saveas(gcf,fullfile(FigPath,'Figure_S6B_5HT_RxTime'),'jpeg');