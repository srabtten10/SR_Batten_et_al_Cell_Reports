...
%Run linear mixed-effects analysis (Model 15) for plots Figure S5B to look
%at left v right hemisphere for neutral words
...
for cc = 1:size(Rx_Time_Positive,1)
    Rx_Times_vert{cc,1} = vertcat(Rx_Time_Neutral{cc,1});
end
...
for cc = 1:size(PickSub,2)
    valence_vert{cc,1} = vertcat(neut{cc,1});
end
...
for cc = 1:size(PickSub,2)
    accuracy_vert{cc,1} = vertcat(neut_acc{cc,1});
end
...
for cc = 1:size(pos_Peak_Change,1)
    ...
    Peak_Subject{cc,gg} = vertcat(neut_Peak_Change{cc,gg});
    end
end
