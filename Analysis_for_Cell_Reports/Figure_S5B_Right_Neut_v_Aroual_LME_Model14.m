...
%Run linear mixed-effects analysis (Model 14) for plots Figure S5B to look
%at right hemisphere only across neutral and arousal
...
PickSub = [3,4,6]; %makes analyzing different people modular 
...
for cc = 1:size(PickSub,2)
    accuracy_vert{cc,1} = vertcat(pos_acc{cc,1},neg_acc{cc,1},neut_acc);
end
...
datanames   = {'Subject','Arousal','RxTime',...
               'Peak'};
for cc = 1:size(Model_Peak_Change,2)    
    Model_Table = table(categorical(Model_Subject),categorical(Model_Salience),Model_Rx_Time,Model_Peak_Change(:,cc),...
...
Relative_Model_Names = cell({'Peak ~ 1 + Arousal * RxTime + (1 + Arousal * RxTime|Subject)'});
                         
Relative_NoRand_Model_Names = cell({'Peak ~ 1 + Arousal * RxTime + (1|Subject)'});
