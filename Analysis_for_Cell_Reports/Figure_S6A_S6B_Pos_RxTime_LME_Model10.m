...
%Run linear mixed-effects analysis (Model 10) and plots Figure S6A and S6B
%positive valence reaction times
...
%With Random Effects
formulaz = 'Peak ~ 1 + RxTime * Hemisphere + (1 + RxTime * Hemisphere|Subject)';
lme_Relative_AUC = fitglme(Model_Table,formulaz,'distribution','normal','Link','identity','FitMethod','Laplace');
%Without Random Effects
formulaz = 'Peak ~ 1 + RxTime * Hemisphere + (1|Subject)';
...
Relative_Model_Names = cell({'Peak ~ 1 + RxTime * Hemisphere + (1 + RxTime * Hemisphere|Subject)'});
                         
Relative_NoRand_Model_Names = cell({'Peak ~ 1 + RxTime * Hemisphere+ (1|Subject)'});
