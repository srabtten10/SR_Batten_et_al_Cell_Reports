...
%Run linear mixed-effects analysis (Model 9) and plots Figure 5D
...
for cc = 1:size(PickSub,2)
    ...
    Pos(cc,2) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == 1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    
    
    Neg(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 0,1),'omitnan');
    Neg(cc,2) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == -1 & Hemisphere_vert{cc,1} == 1,1),'omitnan');
    
end


%DA Mean
mean_L(2,1) = mean(Pos(:,1),'omitnan');
mean_L(1,1) = mean(Neg(:,1),'omitnan');

%Group SEM:DA
SEM_L(2,1) = std(Pos(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,1))))));
SEM_L(1,1) = std(Neg(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,1))))));

%SE Mean
mean_R(2,1) = mean(Pos(:,2),'omitnan');
mean_R(1,1) = mean(Neg(:,2),'omitnan');

%Group SEM:SE
SEM_R(2,1) = std(Pos(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Pos(:,2))))));
SEM_R(1,1) = std(Neg(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Neg(:,2))))));
...
saveas(gcf,fullfile(FigPath,'Figure_5D_DA_ACC'),'jpeg');