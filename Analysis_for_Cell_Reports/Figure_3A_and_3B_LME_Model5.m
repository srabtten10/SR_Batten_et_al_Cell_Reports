...
%Run linear mixed-effects analysis (Model 5) and plots Figure 3A and 3B
...
%% Backward-looking smoothing puts 'artificial' zeros in first four spots...
...
%% ** GET VARIABLES READY FOR LINEAR MIXED-EFFECTS (LME) MODELING** %%
...
for cc = 1:size(Model_Peak_Change,2)    
    Model_Table = table(categorical(Model_Subject),Model_Rx_Time,...
                        Model_Valence_Rating,Model_Arousal_Rating,Model_Peak_Change(:,cc),'VariableNames',datanames);

%With Random Effects for both intercept and slope
formulaz = 'Peak ~ 1 + (Valence_Num + Arousal_Num) * RxTime + (1 + (Valence_Num + Arousal_Num) * RxTime|Subject)';
...
formulaz = 'Peak ~ 1 + (Valence_Num + Arousal_Num) * RxTime + (1|Subject)';
...
Relative_Model_Names = cell({'Peak ~ 1 + (Valence_Num + Arousal_Num) * RxTime + (1 + (Valence_Num + Arousal_Num) * RxTime|Subject)'});

Relative_NoRand_Model_Names = cell({'Peak ~ 1 + (Valence_Num + Arousal_Num) * RxTime + (1|Subject)'});
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Pos(cc,1) = mean(Peak_Subject{cc,1}(salience_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,2) = mean(Peak_Subject{cc,2}(salience_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,3) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == 1,1),'omitnan');
    
    
    Neut(cc,1) = mean(Peak_Subject{cc,1}(salience_vert{cc,1} == -1,1),'omitnan');
    Neut(cc,2) = mean(Peak_Subject{cc,2}(salience_vert{cc,1} == -1,1),'omitnan');
    Neut(cc,3) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == -1,1),'omitnan');
end
...
saveas(gcf,fullfile(FigPath,'Figure_3A_Random_VIM_NE'),'jpeg');

%% Figure 3B valence plots for DA/5HT
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Pos(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == 1,1),'omitnan');
    Pos(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == 1,1),'omitnan');
    
    Neg(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == -1,1),'omitnan');
    Neg(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == -1,1),'omitnan');
    Neg(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == -1,1),'omitnan');
    
    Neut(cc,1) = mean(Peak_Subject{cc,1}(valence_vert{cc,1} == 0,1),'omitnan');
    Neut(cc,2) = mean(Peak_Subject{cc,2}(valence_vert{cc,1} == 0,1),'omitnan');
    Neut(cc,3) = mean(Peak_Subject{cc,3}(valence_vert{cc,1} == 0,1),'omitnan');
end
...
saveas(gcf,fullfile(FigPath,'Figure_3B_Random_VIM_DA_5HT'),'jpeg');