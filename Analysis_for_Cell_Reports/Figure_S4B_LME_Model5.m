...
%Run linear mixed-effects analysis (Model 5) and plots Figure S4B
...
%% Arousal plot for DA and 5HT; Figure S4B
...
%Get indexes 
for cc = 1:size(PickSub,2)
    Aro(cc,1) = mean(Peak_Subject{cc,1}(salience_vert{cc,1} == 1,1),'omitnan');
    Aro(cc,2) = mean(Peak_Subject{cc,2}(salience_vert{cc,1} == 1,1),'omitnan');
    Aro(cc,3) = mean(Peak_Subject{cc,3}(salience_vert{cc,1} == 1,1),'omitnan');
...
%DA Mean
mean_DA(2,1) = mean(Aro(:,1),'omitnan');
mean_DA(1,1) = mean(Neut(:,1),'omitnan');

%Group SEM:DA
SEM_DA(2,1) = std(Aro(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Aro(:,1))))));
SEM_DA(1,1) = std(Neut(:,1),1,'omitnan')/(sqrt(length(find(~isnan(Neut(:,1))))));

%SE Mean
mean_SE(2,1) = mean(Aro(:,2),'omitnan');
mean_SE(1,1) = mean(Neut(:,2),'omitnan');

%Group SEM:SE
SEM_SE(2,1) = std(Aro(:,2),1,'omitnan')/(sqrt(length(find(~isnan(Aro(:,2))))));
...
saveas(gcf,fullfile(FigPath,'Figure_S4B_Blocked_VIM_DA_5HT'),'jpeg');