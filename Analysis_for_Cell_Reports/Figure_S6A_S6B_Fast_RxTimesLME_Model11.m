...
%Run linear mixed-effects analysis (Model 11) for plots S6A and S6B
%categorical negative vs postive words for fast reaction times 
...
%% Get Reaction Time Labels based on Terciles 
...
%Get indexes 
for cc = 1:size(PickSub,2)
    for rr = 1:size(Rx_Times_vert{cc,1}(:,1),1)
    if Rx_Times_vert{cc,1}(rr,1) < Qantiles{cc,1}(1,1)
        Rx_Time_Label{cc,1}(rr,1) = 1; 
    elseif Rx_Times_vert{cc,1}(rr,1) >= Qantiles{cc,1}(1,1) & Rx_Times_vert{cc,1}(rr,1) <= Qantiles{cc,1}(1,2)
        Rx_Time_Label{cc,1}(rr,1) = 0;
    elseif  Rx_Times_vert{cc,1}(rr,1) > Qantiles{cc,1}(1,2)
        Rx_Time_Label{cc,1}(rr,1) = -1;
    end
    end
end

Model_Reaction_Time_Label = vertcat(Rx_Time_Label{:});  
...
for cc = 1:size(PickSub,2)
    if NT_Final(PickSub(cc)).Region == 'LACC'
        Hemisphere{cc,1} = repelem(0,88)'; 
    else 
        Hemisphere{cc,1} = repelem(1,88)';  
    end
end
...
datanames = {'Subject','Valence','Hemisphere',...
               'Peak','Reaction_Time_Label'};
   
Model_Table = table(categorical(Model_Subject),categorical(Model_Valence),categorical(Model_Hemisphere),Model_Peak_Change(:,2),categorical(Model_Reaction_Time_Label),'VariableNames',datanames);

%% Get table with just fast reaction times

   fast_data = Model_Table.Reaction_Time_Label == '1' ;

   Model_Rx_Table = Model_Table(fast_data,:);

%% Run the models

%With Random Effects
formulaz = 'Peak ~ 1 + Valence * Hemisphere  + (1 + Valence * Hemisphere |Subject)';
lme_Relative_AUC = fitglme(Model_Rx_Table,formulaz,'distribution','normal','Link','identity','FitMethod','Laplace');
%Without Random Effects
formulaz = 'Peak ~ 1 + Valence * Hemisphere + (1|Subject)';
lme_Relative_NoRand_AUC = fitglme(Model_Rx_Table,formulaz,'distribution','normal','Link','identity','FitMethod','Laplace');
...
Relative_Model_Names = cell({'Peak ~ 1 + Valence * Hemisphere + (1 + Valence  * Hemisphere|Subject)'});
                         
Relative_NoRand_Model_Names = cell({'Peak ~ 1 + Valence * Hemisphere + (1|Subject)'});
