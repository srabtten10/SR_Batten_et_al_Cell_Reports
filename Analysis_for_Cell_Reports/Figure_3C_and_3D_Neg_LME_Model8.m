...
%negative valence (for plots Figure 3C and 3D)
...
for cc = 1:size(Rx_Time_Positive,1)
    Rx_Times_vert{cc,1} = vertcat(Rx_Time_Negative{cc,1},Rx_Time_Negative{cc,1});
end
...
for cc = 1:size(PickSub,2)
    valence_vert{cc,1} = vertcat(neg{cc,1},neg{cc,1});
end
...
for cc = 1:size(PickSub,2)
    accuracy_vert{cc,1} = vertcat(neg_acc{cc,1},neg_acc{cc,1});
end
...
for cc = 1:size(pos_Peak_Change,1)
    Peak_Subject{cc,1} = vertcat(neg_Peak_Change{cc,1},...
                                      neg_Peak_Change{cc,2});
end
